  ...
  async start() {
    ...
    this.server = createServer(async (req, res) => {
      try {
        ...
        if (req.method === "GET" && req.url === "/api/health") { res.writeHead(200, jsonHeaders); res.end(JSON.stringify({ ok: true })); return; }
        if (req.method === "GET" && req.url === "/api/gates") { res.writeHead(200, jsonHeaders); res.end(JSON.stringify(await this._gates())); return; }
        const match = req.url.match(/^\/api\/gates\/([^/]+)(\/respond)?$/);
        if (match && req.method === "GET") { const gate = await this._gate(decodeURIComponent(match[1])); if (!gate) throw Object.assign(new Error("not found"), { code: 404 }); res.writeHead(200, jsonHeaders); res.end(JSON.stringify(gate)); return; }
        if (match && match[2] && req.method === "POST") { let body = ""; for await (const chunk of req) body += chunk; const result = await this._respond(decodeURIComponent(match[1]), JSON.parse(body)); res.writeHead(200, jsonHeaders); res.end(JSON.stringify(result)); return; }
        res.writeHead(404); res.end("not found");
      } catch (error) { res.writeHead(error.code === 404 ? 404 : 400, jsonHeaders); res.end(JSON.stringify({ error: error.message })); }
    });
