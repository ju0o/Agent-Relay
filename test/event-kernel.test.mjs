...
// ?? D-18 pending PM queue filtering ??????????????????????????????????????
  ...
  const o2 = await evk.recordRunResultReceived(TEST_ROOT, project, { ...base, occurredAt: tInfo }); // INFO but PM-required
  const o3 = await evk.recordPmReviewRequired(TEST_ROOT, project, { ...base, occurredAt: tWarn });
  ...
  check(orderPos(o3.eventId) < orderPos(o2.eventId), 'D-19 WARNING before INFO');
